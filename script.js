...
// Eye icon helpers
const eyeOffPath = '<path d="M2 12s4-7 10-7 10 7 10 7-4 7-10 7S2 12 2 12z"/><circle cx="12" cy="12" r="3"/><line x1="4" y1="4" x2="20" y2="20"/>';
const eyeOnPath = '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8S1 12 1 12z"/><circle cx="12" cy="12" r="3"/>';

function initEye(eyeSpanId, iconId, inputId) {
  const eyeSpan = document.getElementById(eyeSpanId);
  const icon = document.getElementById(iconId);
  const input = document.getElementById(inputId);
  
  if (!eyeSpan || !icon || !input) return;
  ...
  eyeSpan.addEventListener("click", function(e) {
    ...
    icon.innerHTML = isPassword ? eyeOnPath : eyeOffPath;
  });
}

// Initialize all eye toggles
...
loginBtn?.addEventListener("click", async () => {
  ...
  if (!un || !pw) {
    setStatus(loginStatus, "Please enter username and password.", "error");
    return;
  }
...
aregVerify?.addEventListener("click", async () => {
  ...
  if (!entered) {
    aregError.style.color = "#c0392b";
    aregError.textContent = "Please enter the code.";
    return;
  }
  ...
  try {
    ...
    if (!snap.exists()) {
      aregError.style.color = "#c0392b";
      aregError.textContent = "No code found. Refresh and try again.";
      return;
    }
...
aregBtn?.addEventListener("click", async () => {
  if (!aregVerified) {
    setStatus(aregStatus, "Please complete authentication first.", "error");
    return;
  }
  ...
  if (!fn || !ln || !un || !pw || !cpw) {
    setStatus(aregStatus, "Please fill in all fields.", "error");
    return;
  }
  if (pw.length < 6) {
    setStatus(aregStatus, "Password must be at least 6 characters.", "error");
    return;
  }
  if (pw !== cpw) {
    setStatus(aregStatus, "Passwords do not match.", "error");
    return;
  }
...
if (logoutModal) {
  logoutModal.addEventListener("click", function(e) {
    if (e.target === logoutModal || e.target.classList.contains("modal-overlay")) {
      hideLogoutModal();
    }
  });
}
...
empregBtn?.addEventListener("click", async () => {
  ...
  if (!empId) {
    setStatus(empregStatus, "Please enter an Employee ID.", "error");
    return;
  }
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (isCompleted) {
        setStatus(empregStatus, `❌ ID "${empId}" is already registered and completed. Cannot register again.`, "error");
      } else {
        setStatus(empregStatus, `⚠️ ID "${empId}" is already pending registration. Cannot register again.`, "warning");
      }
    } else {
      await setDoc(docRef, { UserID: empId, status: "pending", registeredAt: serverTimestamp() });
      setStatus(empregStatus, `✅ Employee ID "${empId}" registered successfully! Waiting for mobile app completion.`, "success");
      empregInput.value = "";
    }
  } catch (err) {
    ...
    empregBtn.textContent = "Register Employee";
  ...
  ridUnsubscribe = onSnapshot(
    ...
    (snap) => {
      ...
      snap.forEach(s => {
        ...
        const isCompleted = !!(data.First_Name || data.firstName);
      ...
      document.getElementById("rid-total-count").textContent = allCompleted.length + allPending.length;
...
function renderCompleted(users) {
  ...
  users.forEach(u => {
    ...
    card.innerHTML = `
      <div class="emp-avatar">${escapeHtml(initials.toUpperCase())}</div>
      <div class="emp-name">${escapeHtml(fn)} ${mn ? escapeHtml(mn) + " " : ""}${escapeHtml(ln)}</div>
      ${mn ? `<div class="emp-mid">M.I.: ${escapeHtml(mn)}</div>` : ""}
      <div class="emp-id">ID: ${escapeHtml(u.UserID || u.id)}</div>
      ${u.Username ? `<div class="emp-username">@${escapeHtml(u.Username)}</div>` : ""}
      ${u.Position ? `<div class="emp-position">${escapeHtml(u.Position)}</div>` : ""}
    `;
...
function renderPending(users) {
  ...
  if (!users.length) {
    pendingList.innerHTML = `<p class="no-results" style="text-align:center;padding:40px 0;">No pending registrations.</p>`;
    return;
  }
  users.forEach(u => {
    ...
    item.innerHTML = `
      <div class="pending-item-left">
        ...
        <div>
          <div class="pending-id">${escapeHtml(u.UserID || u.id)}</div>
